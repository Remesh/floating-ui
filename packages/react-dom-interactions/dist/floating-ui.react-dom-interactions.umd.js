  ...
  function FloatingFocusManager(_ref) {
    ...
    const getTabbableElements = React__namespace.useCallback(() => {
      return orderRef.current.map(type => {
        ...
        if (type === 'content') {
          ...
          return Array.from(((_refs$floating$curren = refs.floating.current) == null ? void 0 : _refs$floating$curren.querySelectorAll(SELECTOR)) ?? []);
        }
    ...
    React__namespace.useEffect(() => {
      ...
      function onFocusOut(event) {
        var _refs$floating$curren2, _getAncestors;

        const relatedTarget = event.relatedTarget;
        const focusMovedOutsideFloating = !((_refs$floating$curren2 = refs.floating.current) != null && _refs$floating$curren2.contains(relatedTarget));
